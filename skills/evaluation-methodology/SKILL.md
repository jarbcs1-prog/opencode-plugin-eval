...
The `eval-judge` agent reads the SKILL.md and any `references/` files, then scores four dimensions using anchored rubrics (see [references/rubrics.md](references/rubrics.md)):
...
**Problem:** Without a meaningful description, the OpenCode plugin system cannot determine when to invoke the skill. The skill becomes invisible to autonomous invocation.
...
Example: "Use this skill when measuring plugin quality, interpreting score reports or explaining badge thresholds to a team."
