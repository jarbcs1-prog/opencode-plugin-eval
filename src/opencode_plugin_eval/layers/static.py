...
class StaticAnalyzer:
    ...
    def _detect_skill_anti_patterns(self, skill: ParsedSkill) -> list[AntiPattern]:
        ...
        if skill.has_references:
            ...
            # Known example/documentation filenames that are not real references
            example_filenames = {"filename.md", "filename", "examples.md", "config.md", "guide.md"}
            for linked in linked_refs:
                linked_name = Path(linked).name.lower()
                if linked_name in example_filenames:
                    continue  # Skip documentation examples
                if linked_name not in existing:
                    patterns.append(
                        AntiPattern(
        ...
        for ref in skill.cross_references:
            # Skip common false positives from table cells and documentation
            if ref in {"agents", "skills", "plugins", "examples"}:
                continue
